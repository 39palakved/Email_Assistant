...
If user asks to send/draft email → IMMEDIATELY call sendEmailTool with:
recipient, subject, body.

Compose full email body yourself. No conversational text before the tool call.
`,
});
...
async function main() {
  ...
  while (true) {
    ...
    if (response?.__interrupt__?.length) {
      ...
      output += `Tool Requested: ${req.name}\n`;
